-module(graphql@multi_query_with_vars).
-compile([no_auto_import, nowarn_unused_vars, nowarn_unused_function, nowarn_nomatch, inline]).
-define(FILEPATH, "src/graphql/multi_query_with_vars.gleam").
-export([info_decoder/0, character_decoder/0, characters_decoder/0, location_decoder/0, episode_decoder/0, multi_query_with_vars_response_decoder/0, multi_query_with_vars/5]).
-export_type([characters/0, info/0, character/0, location/0, episode/0, multi_query_with_vars_response/0]).
...
-type multi_query_with_vars_response() :: {multi_query_with_vars_response,
        gleam@option:option(characters()),
        gleam@option:option(location()),
        gleam@option:option(list(episode()))}.

-file("src/graphql/multi_query_with_vars.gleam", 31).
...
-file("src/graphql/multi_query_with_vars.gleam", 45).
...
-file("src/graphql/multi_query_with_vars.gleam", 13).
...
-file("src/graphql/multi_query_with_vars.gleam", 59).
        ...
        gleam@result:'try'(
            ...
            fun(Id) ->
                gleam@result:'try'(
                    ...
                    fun(Name) -> {ok, {location, Id, Name}} end
                )
            end
        )
    end.

-file("src/graphql/multi_query_with_vars.gleam", 76).
        ...
        gleam@result:'try'(
            ...
            fun(Id) ->
                gleam@result:'try'(
                    ...
                    fun(Name) -> {ok, {episode, Id, Name}} end
                )
            end
        )
    end.

-file("src/graphql/multi_query_with_vars.gleam", 97).
-spec multi_query_with_vars_response_decoder() -> fun((gleam@dynamic:dynamic_()) -> {ok,
...
-file("src/graphql/multi_query_with_vars.gleam", 124).
-spec multi_query_with_vars(
    binary(),
    integer(),
    binary(),
    binary(),
    list(integer())
) -> {ok, multi_query_with_vars_response()} | {error, binary()}.
multi_query_with_vars(Endpoint, Page, Name, Location_id, Episode_ids) ->
    Query = <<"query MultiQueryWithVars($page: Int, $name: String, $locationId: ID!, $episodeIds: [Int!]!) { characters(page: $page, filter: { name: $name }) { info { count } results { name } } location(id: $locationId) { id name } episodesByIds(ids: $episodeIds) { id name } }"/utf8>>,
    Variables = gleam@json:object(
        [{<<"page"/utf8>>, gleam@json:int(Page)},
            {<<"name"/utf8>>, gleam@json:string(Name)},
            {<<"locationId"/utf8>>, gleam@json:string(Location_id)},
            {<<"episodeIds"/utf8>>,
                gleam@json:array(Episode_ids, fun gleam@json:int/1)}]
    ),
                    ...
                    gleam@result:'try'(
                        ...
                        fun(Json_value) ->
                            gleam@result:'try'(
                                ...
                                fun(Data_field) ->
                                    _pipe@7 = (multi_query_with_vars_response_decoder(
